(() => {
  ...
  function buildFoodItemRow(item = {}) {
    ...
    row.innerHTML = `
      ...
      <div class="grid gap-3 pr-10">
        <div>
          <input type="text" class="w-full px-4 py-2.5 border-2 border-gray-200 rounded-xl focus:border-emerald-400 focus:outline-none transition-colors text-sm" placeholder="Item name (e.g., Rice, Bread)" required value="${escapeAttribute(item.itemName || item.name || "")}">
        </div>
        <div class="grid grid-cols-12 gap-3">
          ...
          <div class="col-span-6 sm:col-span-3">
            <input type="number" step="0.01" class="w-full px-4 py-2.5 border-2 border-gray-200 rounded-xl focus:border-emerald-400 focus:outline-none transition-colors text-sm" placeholder="Qty" required value="${escapeAttribute(item.quantity || "")}">
          </div>
          <div class="col-span-6 sm:col-span-3">
            <input type="text" class="w-full px-4 py-2.5 border-2 border-gray-200 rounded-xl focus:border-emerald-400 focus:outline-none transition-colors text-sm" placeholder="Unit (e.g. kg)" required value="${escapeAttribute(item.unit || "")}">
          </div>
        </div>
      </div>
    `;
  ...
  function getFoodItems({ strict = false } = {}) {
    return getFoodRows().map((row) => {
      ...
      if (strict && (!itemName || !category || !quantity || !unit)) return null;
      return { itemName, category, quantity, unit };
    }).filter(Boolean);
  }

  function calculateEstimatedServings(items = []) {
    const estimated = items.reduce((total, item) => total + estimateServingsFromQuantity(item.quantity, item.unit), 0);
  ...
  function getDraftPayload() {
    ...
    const payload = {
      ...
      pickup_datetime: getFieldValue(elements.pickupTimeInput),
  ...
  function applyDraft(draft) {
    ...
    const safeItems = Array.isArray(draft.items) ? draft.items : Array.isArray(draft.foodItems) ? draft.foodItems : [];
    ...
    if (elements.pickupTimeInput && (draft.pickup_datetime || draft.pickupTime)) {
      elements.pickupTimeInput.value = draft.pickup_datetime || draft.pickupTime;
    }
  ...
  async function handleSubmit(event) {
    ...
    const donationData = {
      ...
      zip: getFieldValue(elements.form.querySelector('[name="zip"]')),
      pickup_datetime: getFieldValue(elements.pickupTimeInput),
      priority: getFieldValue(elements.form.querySelector('[name="priority"]')) || 'medium',
      notes: getFieldValue(elements.form.querySelector('[name="notes"]')),
      impact: { estimatedServings: calculateEstimatedServings(items) }
    };
