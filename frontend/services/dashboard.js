...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    renderDashboard({ role, totalDonations, totalServings, communitiesServed, donations, statsRes });
...
function renderDashboard({ role, totalDonations, totalServings, communitiesServed, donations, statsRes }) {
...
// --------------------------------------------------------------------------
  console.log('[Dashboard] Stats Payload:', {
    role,
    totalDonations: roleTotalDonations,
    totalServings: roleTotalServings,
    communitiesServed: resolvedCommunitiesServed
  });
// --------------------------------------------------------------------------
  ...
  // NEW: Trend indicators and additional stats if available in statsRes
  setText("totalDonationsTrend", "Live Update");
  setText("totalServingsTrend", "Live Update");
  setText("activeDonors", formatNumber(statsRes?.data?.activeDonors || 0));
  ...
  container.innerHTML = items
    ...
    .map((donation) => {
      const itemName = escapeHtml(
        donation.items?.[0]?.itemName || "Food donation"
      );
      const badge = getPriorityBadge(Number(donation?.priorityScore) || 0);

      const city = escapeHtml(
        donation.city || "No location"
      );

      const quantity = escapeHtml(
        donation.items?.[0]?.quantity || "Quantity not provided"
      );
